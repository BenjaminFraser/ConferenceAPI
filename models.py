...
# Define a conference object with the desired fields our conferences require.
...
# Define a conference form class, allowing form conference creation
...
# Since more than one conference is displayed at one time during conference
# overview, we need to create a multiple conference form message class.
# This repeated field allows each conference to be stored as an object.
...
# Allow multiple conferences query form classes to be queried, with each
# query stored as an object, in the filters repeated field.
...
# define a session model to enable session objects. 
...
class SessionForm(messages.Message):
    ...
    creatorUserId   = messages.StringField(8)
    websafeKey      = messages.StringField(9)
    creatorDisplayName = messages.StringField(10)

# Create SessionForms, for multiple sessions to be displayed.
class SessionForms(messages.Message):
    """SessionForms -- multiple Session outbound form message"""
    items = messages.MessageField(SessionForm, 1, repeated=True)

# Create a message for storing session queries.
...
# Allow multiple session queries to be carried out.
...
# StringMessage message class for creating memcache announcements.
